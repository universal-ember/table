...
module("Plugins | RowSelection", function (hooks) {
  ...
  module("with default options", function (hooks) {
    ...
    test("rendering with invalid arguments is not allowed", async function (assert) {
      ...
      setupOnerror((error) => {
        ...
        assert.true(
          /Assertion Failed: selection, onSelect, and onDeselect are all required arguments for the RowSelection plugin/.test(
            errorStr,
          ),
        );
      });
  ...
  module("with all required options", function (hooks) {
    ...
    test("it works (the non accessible way by clicking <tr>s", async function (assert) {
      ...
      debugAssert(
        "rows not found",
        rows[0] instanceof HTMLElement && rows[1] instanceof HTMLElement,
      );
    ...
    module(
      "helpers work as expected (and would be used for accessible implementations)",
      function () {
          ...
          await render(
            <template>
              ...
              <button
                id="the-helper-1"
                type="button"
                {{on "click" (fn toggle first)}}
              >click me</button>
              <button
                id="the-helper-2"
                type="button"
                {{on "click" (fn toggle second)}}
              >click me</button>
            </template>,
          );
          ...
          await render(
            <template>
              ...
              <button
                id="select-1"
                type="button"
                {{on "click" (fn select first)}}
              >click me</button>
              <button
                id="select-2"
                type="button"
                {{on "click" (fn select second)}}
              >click me</button>
              <button
                id="deselect-1"
                type="button"
                {{on "click" (fn deselect first)}}
              >click me</button>
              <button
                id="deselect-2"
                type="button"
                {{on "click" (fn deselect second)}}
              >click me</button>
            </template>,
          );
