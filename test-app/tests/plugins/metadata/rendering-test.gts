...
module("Plugins | metadata", function (hooks) {
  ...
  module("Reactivity", function (hooks) {
        ...
        <template>
          {{#each ctx.table.columns as |column|}}
            <span class="{{column.key}}">
              <span class="isRad">{{step
                  column.key
                  "isRad"
                  (forColumn column "isRad")
                }}</span>
              <span class="isMissing">{{step
                  column.key
                  "isMissing"
                  (forColumn column "isMissing")
                }}</span>
              <span class="doingOneHandedFlips">{{step
                  column.key
                  "flips"
                  (forColumn column "doingOneHandedFlips")
                }}</span>
            </span>
          {{/each}}
          ...
          <span class="table">
            <span class="foo">{{step
                "table"
                "foo"
                (forTable ctx.table "foo")
              }}</span>
            <span class="bar">{{step
                "table"
                "bar"
                (forTable ctx.table "bar")
              }}</span>
          </span>
        </template>,
      );
    ...
    test("it works, dynamically", async function (assert) {
      ...
      assert.dom(find(".table .foo")).hasText("2");
      assert.verifySteps(
        ["B@isRad: false", "B@isMissing: undefined", "B@flips: true"],
      ...
      assert.dom(find(".table .foo")).hasText("2");
      assert.verifySteps(
        ["B@isRad: false", "B@isMissing: undefined", "B@flips: false"],
        `all of column B's properties are re-evaluated, because this test does not do finer-grained reactivity. ` +
          `but that is possible, if the user wishes`,
      );
      ...
      assert.verifySteps(
        ["table@foo: 3", "table@bar: undefined"],
        `all of column B's properties are re-evaluated, because this test does not do finer-grained reactivity. ` +
          `but that is possible, if the user wishes`,
      );
    });
