...
module("Plugins | resizing | column options", function (hooks) {
  ...
  module("isResizable: false", function (hooks) {
        ...
        columns: () => [
          {
            ...
            key: "A",
            pluginOptions: [
              ColumnResizing.forColumn(() => ({ isResizable: false })),
            ],
          },
          { name: "B", key: "B" },
          { name: "C", key: "C" },
    ...
    test(`option propogates to the meta's value`, async function (assert) {
      ...
      assert.strictEqual(
        meta.forColumn(columns[0], ColumnResizing).isResizable,
        false,
      );
      assert.strictEqual(
        meta.forColumn(columns[1], ColumnResizing).isResizable,
        true,
      );
      assert.strictEqual(
        meta.forColumn(columns[2], ColumnResizing).isResizable,
        true,
      );
      assert.strictEqual(
    ...
    test(`[hasResizeHandle] there are fewer resize handles than there would be if all columns were resizable`, async function (assert) {
      ...
      assert
        ...
        .exists(
          { count: 1 },
          "there is one boundary between columns for adjusting size",
        );
    });
  });
