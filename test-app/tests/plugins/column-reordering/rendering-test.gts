...
module("Plugins | columnReordering", function (hooks) {
  ...
  let ctx: Context;
  let getColumnOrder = () =>
    findAll("thead tr .name")
      .map((x) => {
        assert(
          "expected element to exist and have innerText",
    ...
    <template>
      ...
      <div>
        {{#each (columns.for this.table ColumnVisibility) as |column|}}
          {{column.name}}:
          <button
            class="hide {{column.key}}"
            type="button"
            {{on "click" (fn hide column)}}
          >
            Hide
          </button>
          <button
            class="show {{column.key}}"
            type="button"
            {{on "click" (fn show column)}}
          >
            Show
          </button>
          <br />
  ...
  module("as a solo plugin", function (hooks) {
              ...
              <thead>
                <tr>
                  {{#each (columns.for ctx.table) as |column|}}
                    <th
                      class="{{column.key}}"
                      {{ctx.table.modifiers.columnHeader column}}
                    >
                      <button
                        class="left"
                        type="button"
    ...
    test("everything is visible and in the original order", async function (assert) {
      ...
      assert.strictEqual(getColumnOrder(), "A B C D", "Initial order");
    });
  ...
  module("with no options specified", function (hooks) {
    ...
    test("hiding and showing a column preserves order", async function (assert) {
      ...
      assert.strictEqual(
        getColumnOrder(),
        "A",
        "only column A remains, both first and last",
      );
      ...
      assert.strictEqual(
        getColumnOrder(),
        "B A",
        "column B has returned, and it is first",
      );
