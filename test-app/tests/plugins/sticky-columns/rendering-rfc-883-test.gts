...
module("Plugins | StickyColumns | RFC#883 work-around", function (hooks) {
  ...
  module("with default options", function (hooks) {
    ...
    test("no columns are sticky", async function (assert) {
      ...
      assert.ok(
        isAbout(left, 1),
        `A's left edge (@ ${left}) matches the left edge of the container`,
      );
      ...
      assert
        ...
        .hasAttribute(
          ...
          "min-width renders correctly as style",
        );
      ...
      assert.ok(
        isAbout(left, -200),
        `A's left edge (@ ${left}) moved, so it's not sticky`,
      );
    });
  });
  ...
  module("the left column can be sticky", function (hooks) {
      ...
      columns = [
        {
          name: "column A",
          key: "A",
          pluginOptions: [leftSticky(), minWidth()],
        },
    ...
    test("the left column does not change position during scrolling", async function (assert) {
      ...
      assert.ok(
        isAbout(leftA, 1),
        `A's left edge (@ ${leftA}) matches the left edge of the container`,
      );

      let leftB = leftPositionOf("B");
      assert.ok(
        isAbout(leftB, 201),
        `B's left edge (@ ${leftB}) is one column's width right of A`,
      );
      ...
      assert.ok(
        isAbout(leftA, 0),
        `A's left edge (@ ${leftA}) is stuck to the left`,
      );
      leftB = leftPositionOf("B");
      assert.ok(
        isAbout(leftB, 1),
        `B's left edge (@ ${leftB}) now shares an edge with A, due to scrolling`,
      );
    });
  });
  ...
  module("the right column can be sticky", function (hooks) {
    class RightColumn extends TestSetup {
      columns = [
        ...
        { name: "column F", key: "F", pluginOptions: [minWidth()] },
        {
          name: "column G",
          key: "G",
          pluginOptions: [minWidth(), rightSticky()],
        },
      ];
    }
    ...
    test("the right column does not change position during scrolling", async function (assert) {
      ...
      assert.ok(
        isAbout(rightG, 0),
        `G's right edge (@ ${rightG}) matches the right edge of the container`,
      );
      ...
      assert.ok(
        isAbout(rightG, 0),
        `G's right edge (@ ${rightG}) is stuck to the right`,
      );
      ...
      assert.ok(
        isAbout(rightG, 0),
        `G's right edge (@ ${rightG}) hasn't moved`,
      );

      rightF = rightPositionOf("F");
      assert.ok(
        isAbout(rightF, -200),
        `F's right edge (@ ${rightF}) now shares an edge with G, due to scrolling`,
      );
    });
  });
  ...
  module("2 left columns can be sticky", function (hooks) {
    ...
    test("the 2 left columns do not change position during scrolling", async function (assert) {
      ...
      assert.ok(
        isAbout(leftC, 400),
        `C's left edge (@ ${leftC}) is two column's width right of A`,
      );

      let leftD = leftPositionOf("D");
      assert.ok(
        isAbout(leftD, 600),
        `D's left edge (@ ${leftD}) is three column's width right of A`,
      );
      ...
      assert.ok(
        isAbout(leftB, 200),
        `B's left edge (@ ${leftB}) is stuck to the left, but right of A`,
      );

      leftC = leftPositionOf("C");
      assert.ok(
        isAbout(leftC, 200),
        `C's left edge (@ ${leftC}) has almost scrolled all the way to A's right edge`,
      );

      leftD = leftPositionOf("D");
      assert.ok(
        isAbout(leftD, 400),
        `D's left edge (@ ${leftD}) is now where C's left edge used to be`,
      );
      ...
      assert.ok(
        isAbout(leftB, 200),
        `B's left edge (@ ${leftB}) has not moved.`,
      );

      leftC = leftPositionOf("C");
      assert.ok(
        isAbout(leftC, 0),
        `C's left edge (@ ${leftC}) is almost as left as it can go`,
      );

      leftD = leftPositionOf("D");
      assert.ok(
        isAbout(leftD, 200),
        `D's left edge (@ ${leftD}) is now where C's left edge used to be (again)`,
      );
    });
  });
  ...
  module("2 right columns can be sticky", function (hooks) {
    ...
    test("the 2 right columns do not change position during scrolling", async function (assert) {
      ...
      assert.ok(
        isAbout(rightE, 200),
        `E's right edge (@ ${rightE}) is one column's width beyond the container's right edge`,
      );
      ...
      assert.ok(
        isAbout(rightF, -200),
        `F's right edge (@ ${rightF}) has not moved`,
      );

      rightE = rightPositionOf("E");
      assert.ok(
        isAbout(rightE, 0),
        `E's right edge (@ ${rightE}) now matches the boundary, but is covered by G`,
      );
      ...
      assert.ok(
        isAbout(rightF, -200),
        `F's right edge (@ ${rightF}) hasn't moved`,
      );

      rightE = rightPositionOf("E");
      assert.ok(
        isAbout(rightE, -400),
        `E's right edge (@ ${rightE}) is now shared with sticky column F's left edge`,
      );

      rightD = rightPositionOf("D");
      assert.ok(
        isAbout(rightD, -600),
        `D's right edge (@ ${rightD}) is all the way on the left`,
      );
    });
  });
  ...
  module("columns on both ends can be sticky", function (hooks) {
      ...
      columns = [
        {
          name: "column A",
          key: "A",
          pluginOptions: [minWidth(), leftSticky()],
        },
        { name: "column B", key: "B", pluginOptions: [minWidth()] },
        { name: "column C", key: "C", pluginOptions: [minWidth()] },
        { name: "column D", key: "D", pluginOptions: [minWidth()] },
    ...
    test("both sticky columns do not change position during scrolling", async function (assert) {
      ...
      assert.ok(
        isAbout(rightF, 200),
        `F's right edge (@ ${rightF}) is still beyond the boundary of the container, but less so`,
      );
    });
  });
});
